...
class GanComponent(nn.Module):
    ...
    def grow(self, n):
        ...
        self.pre_network.add_module(
            "alpha_dropout_" + str(n), nn.AlphaDropout(p=self.dropout_prob)
        )
    ...
    def load_pars(self, path, epoch_num, device):
        if self.component == "generator":
            self.load_state_dict(
                load(
                    path + "/pre_generator_pars_" + epoch_num + ".pt",
                    map_location=device,
                )
            )
        elif self.component == "critic":
            self.load_state_dict(
                load(
                    path + "/pre_critic_pars_" + epoch_num + ".pt", map_location=device
                )
            )
        else:
            print("Error")
...
class Results:
    ...
    def new_record(self, n_pre_layers, results, report_rate):
        if len(self.record.loc[self.record["n_pre_layers"] == n_pre_layers]) != 0:
            epoch = (
                self.record.loc[self.record["n_pre_layers"] == n_pre_layers].iloc[-1, 0]
                + 1 * report_rate
    ...
    def get_top_epochs(self, n_pre_layers):
        ...
        diffs_loss = pd.DataFrame(
            np.column_stack(
                [list(x["epoch"]), list(abs(x["loss_critic"] - x["loss_gen"]))]
            ),
            columns=["epoch", "diffs"],
        )
        ...
        plot = (
            ggplot(None)
            + geom_line(
                data=self.record, color="red", mapping=aes(y="loss_critic", x="epoch")
            )
            + geom_line(
                data=self.record, color="blue", mapping=aes(y="loss_gen", x="epoch")
            )
...
class Environment:
    ...
    def optimise_critic(self, inp_noise, batch_real):
        ...
        pred_fake_clone = (
            pred_fake.detach().clone()
        )  # Make a clone to pass to retain gradients and pass to the optimise_gen function
        ...
        gradients = grad(
            ...
            inputs=interpolates,
            grad_outputs=ones_like(pred_interpolates),
        )  # check
    ...
    def optimise_generator(self, pred_fake, n_sim):
        ...
        norm = linalg.matrix_norm(
            samps, ord=2
        )  # L2 matrix norm; greater value means less similar samples generated
    ...
    def get_scaler(self, n_samples, name_transformer):
        ...
        """
        n = (
            sum(1 for line in open(self.path_dat)) - 1
        )  # number of records in file (excludes header)
        skip = sorted(
            random.sample(range(1, n + 1), n - n_samples)
        ...
        skewed_transformer = Pipeline(
            steps=[
                ...
                )  # allows for -'ve values
            ]
        )
        normal_transformer = Pipeline(steps=[("scaler", RobustScaler())])

        colTrans_early = ColumnTransformer(
            transformers=[
        ...
        dump(
            ext_transformer, open(self.path_results + name_transformer, "wb")
        )  # save transformer
    ...
    def get_batch(self):
        ...
        skip = sorted(
            random.sample(range(1, n + 1), n - self.batch_size)
        )  # the 0-indexed header will not be included in the skip list
        dat = pd.read_csv(self.path_dat, skiprows=skip)
    ...
    def func_epoch(self, return_loss):
        ...
        for i in range(1, self.iter_critic):
            ...
            # train critic
            pred_fake, pred_real, loss_critic = self.optimise_critic(
                inp_noise, batch_real
            )
        # train generator
    ...
    def train(self, n_epochs, verbose=True):
        ...
        if (
            len(
                self.results_record.record.loc[
                    self.results_record.record["n_pre_layers"]
                    == self.generator.n_pre_layers
                ]
            )
            != 0
        ):
        ...
        for i in iterator_tqdm:
            if i % report_rate == 0:
                ...
                self.results_record.new_record(
                    self.generator.n_pre_layers, results, report_rate
                )
        ...
        if verbose == True:
            ...
            time_elapsed = str(prog)[
                str(prog).find("[") + 1 : str(prog).find("<")
            ].strip(" ")
            return time_elapsed
    ...
    def load_pre(self, dropout_prob, seed, preTrained=True, path_prePars=None):
        ...
        self.generator = GanComponent(
            self.n_features, self.init_weights, struct_gen[0], dropout_prob, "generator"
        )
        self.critic = GanComponent(
            self.n_features, self.init_weights, struct_critic[0], dropout_prob, "critic"
        )
        ...
        if flag_loadPars:
            if not preTrained:
                self.generator.load_pars(
                    self.path_pars, f"complete_loop_{id_label}", self.device
                )
                self.critic.load_pars(
                    self.path_pars, f"complete_loop_{id_label}", self.device
                )
                ...
                self.generator.load_pars(
                    path_prePars, f"complete_loop_{id_label}", self.device
                )
                self.critic.load_pars(
                    path_prePars, f"complete_loop_{id_label}", self.device
                )
    ...
    def auto_tune(
        ...
        for i in range(0, len(lr_1), 1):
            lr = lr_1[i]
            self.load_pre(
                dropout_prob, 118, preTrained=preTrained, path_prePars=self.path_prePars
            )
            self.results_record = Results(path=self.path_results, saved_results=False)

            iterator_tqdm = tqdm(
                range(100, max_epochs_1 + 100, 100),
                desc="lr_1 %i out of %i" % (i + 1, len(lr_1)),
            )
            for n_epochs in iterator_tqdm:
                ...
                if losses.iloc[-1] < max_loss_1:
                    if losses.iloc[0] < max_loss_1:
                        ...
                        epoch_final = self.results_record.record["epoch"].iloc[
                            ind_final
                        ]
        ...
        for i in range(0, len(lr_2)):
            ...
            losses = self.results_record.record["loss_critic"].iloc[
                -int(n_epochs_2 / rate_save) :
            ]
            ...
            plot_losses = (
                ggplot(
                    self.results_record.record.iloc[
                        : int(n_epochs_1 / rate_save) + ind_null
                    ]
                )
            ...
            plot_losses.save(
                self.path_results + "/{} tuning_loss".format(lr) + "_retrain" + ".png"
            )

            diff = (
                losses.iloc[: int(diff_epochs / rate_save)].mean()
                - losses.iloc[-int(diff_epochs / rate_save) - ind_null :].mean()
            )
        ...
        best = [
            ...
            for (element1, element2) in zip(
                [n * alpha_instab for n in final_instabs], final_diffs
            )
        ]
    ...
    def tuneAndTrain(self, preTrained=True):
        ...
        lr_1, n_epochs_1, lr_2, results_tuning_1, results_tuning_2 = self.auto_tune(
            **self.tune_params, preTrained=preTrained
        )
        ...
        i_losses = (
            self.results_record.record["n_pre_layers"] == self.generator.n_pre_layers
        )
...
def classification_metrics(
    ...
    for i_classifier in range(0, 2):
        ...
        summary_text = pd.DataFrame(
            columns=["summary", "pos", "col"], index=list(n_iter)
        )
        plot = (
            ggplot()
            ...
            + scale_color_manual(
                values=["m", "b", "g", "y"], limits=["ExpandGAN", "GAN", "SMOTE", "RO"]
            )
        )
        ...
        for i_methods in tqdm(n_iter):
            ...
            if i_methods == 2:
                ...
                if sum(dat_y == 0) < 6:
                    ...
                    def impute_dat(*args):
                        dat_x_imp, dat_y_imp = SMOTE(
                            k_neighbors=k_neighbors
                        ).fit_resample(dat_x, dat_y)
                        return (dat_x_imp, dat_y_imp)
            ...
            if i_methods == 3:
                ...
                def impute_dat(*args):
                    dat_x_imp, dat_y_imp = RandomOverSampler().fit_resample(
                        dat_x, dat_y
                    )
                    return (dat_x_imp, dat_y_imp)
            ...
            # check
            print(
                " %s : %i samples class_1; %i samples class_2"
                % (impute_method, sum(dat_y_imp == 0), sum(dat_y_imp == 1))
            )
            ...
            searcher = GridSearchCV(
                model_val, pars, scoring=scorer, n_jobs=-1, cv=cv, refit=True
            ).fit(dat_x_imp, dat_y_imp)
            ...
            results_classification = pd.concat(
                [results_classification, results_entry], join="inner"
            )
            ...
            plot = plot + geom_line(
                data=dat_plot,
                mapping=aes(x="mean_fpr", y="mean_tpr", color="group"),
                size=1,
            )
            ...
            summary_text.iloc[i_methods, 0] = r"%s (AUC = %0.2f +- %0.2f)" % (
                impute_method_,
                fmean(scores_roc),
                stdev(scores_roc),
            )
        ...
        plot = (
            plot
            + labels.ggtitle("Receiver Operating Characteristic: " + classifier)
            + labs(color="Legend")
