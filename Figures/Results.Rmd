...
```{r}
dat <- read_csv('/Users/cusworsj/Documents/GAN_Paper/Code/gitrepo/results_merged.csv', col_types = 'fffffffdddc')
```

```{r}
dat_notGAN <- dat %>% filter(experiment == 'sim', method == 'SMOTE' | method == 'RO') %>% group_by(method, n_control, class_imbalance) %>% 
  distinct(auc_mean, auc_sd, .keep_all = TRUE) %>% ungroup()
dat_GAN <- dat %>% filter(experiment == 'sim', method == 'ExpandGAN' | (method == 'GAN' & alpha1 == 0) )
dat_GAN <- rbind(dat_GAN, dat %>% filter(experiment == 'sim', method == 'GAN' & alpha1 == 1) %>% distinct(auc_mean, auc_sd, .keep_all = TRUE) %>% ungroup())

dat_graph <- rbind(dat_notGAN, dat_GAN)

dat_notGAN <- dat %>% filter(experiment == 'microarray', method == 'SMOTE' | method == 'RO' | method == 'ClassicGAN' | method == 'ClassicExpand') %>% group_by(method, n_control, class_imbalance) %>% 
  distinct(auc_mean, auc_sd, .keep_all = TRUE) %>% ungroup()
dat_GAN <- dat %>% filter(experiment == 'microarray', method == 'ExpandGAN' | (method == 'GAN' & alpha1 == 0) )
dat_GAN <- rbind(dat_GAN, dat %>% filter(experiment == 'microarray', method == 'GAN' & alpha1 == 1) %>% distinct(auc_mean, auc_sd, .keep_all = TRUE) %>% ungroup())

dat_graph <- rbind(dat_graph, dat_notGAN, dat_GAN)
dat_graph <- dat_graph %>% 
  group_by(method, class_imbalance, experiment, n_control, alpha1, alpha2) %>%
...
p1 <- ggplot(data = dat_graph %>% filter(class_imbalance == 0.4, experiment == 'sim'))+
...
p2 <- ggplot(data = dat_graph %>% filter(class_imbalance == 0.5, experiment == 'sim'))+
  geom_boxplot(aes(x = n_control, upper = (auc_mean + auc_sd) - 0.5, lower = (auc_mean - auc_sd) - 0.5, middle = auc_mean-0.5, 
                   ymax = (auc_mean + auc_sd) - 0.5, ymin = (auc_mean - auc_sd) - 0.5,
                   group = interaction(n_control, alpha1, alpha2, method), col = method), 
               fill = 'white', position = position_dodge2(preserve = 'total', padding = 0.15), stat = 'identity', size = size) +
...
p3 <- ggplot(data = dat_graph %>% filter(class_imbalance == 0.6, experiment == 'sim'))+
  geom_boxplot(aes(x = n_control, upper = (auc_mean + auc_sd) - 0.5, lower = (auc_mean - auc_sd) - 0.5, middle = auc_mean-0.5, 
                   ymax = (auc_mean + auc_sd) - 0.5, ymin = (auc_mean - auc_sd) - 0.5,
                   group = interaction(n_control, alpha1, alpha2, method), col = method), 
               fill = 'white', position = position_dodge2(preserve = 'total', padding = 0.15), stat = 'identity', size = size) +
...
p4 <- ggplot(data = dat_graph %>% filter(class_imbalance == 0.4, experiment == 'microarray', !(method == 'ClassicExpand'), !(method == 'ClassicGAN') ))+
  geom_boxplot(aes(x = n_control, upper = (auc_mean + auc_sd) - 0.5, lower = (auc_mean - auc_sd) - 0.5, middle = auc_mean-0.5, 
                   ymax = (auc_mean + auc_sd) - 0.5, ymin = (auc_mean - auc_sd) - 0.5,
                   group = interaction(n_control, alpha1, alpha2, method), col = method), 
               fill = 'white', position = position_dodge2(preserve = 'total', padding = 0.15), stat = 'identity', size = size) +
...
p5 <- ggplot(data = dat_graph %>% filter(class_imbalance == 0.5, experiment == 'microarray', !(method == 'ClassicExpand'), !(method == 'ClassicGAN')))+
  geom_boxplot(aes(x = n_control, upper = (auc_mean + auc_sd) - 0.5, lower = (auc_mean - auc_sd) - 0.5, middle = auc_mean-0.5, 
                   ymax = (auc_mean + auc_sd) - 0.5, ymin = (auc_mean - auc_sd) - 0.5,
                   group = interaction(n_control, alpha1, alpha2, method), col = method), 
               fill = 'white', position = position_dodge2(preserve = 'total', padding = 0.15), stat = 'identity', size = size) +
...
stat_test %>% filter((method1 %in% methods | method2 %in% methods)) %>% filter(!(method1 %in% not_methods | method2 %in% not_methods)) %>%
  filter(n_control1 == n_control2 & class_imb1 == class_imb2) %>% filter(adj_p <= 0.05) %>% select(-id1, -V1, -V2, -id2) %>%
  arrange(desc(n_control1)) %>% filter(n_control1 == 120) 
```
