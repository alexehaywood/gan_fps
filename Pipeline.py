...
if config["train_pre"]:
    ...
    Env_pre.n_training_samples = (
        sum(1 for line in open(Env_pre.path_dat)) - 1
    )  # number of records in file (excludes header)

    Env_pre.critic = GanComponent(
        Env_pre.n_features,
    ...
    Env_pre.generator = GanComponent(
        Env_pre.n_features,
        Env_pre.init_weights,
        gen_struct[0],
        dropout_prob,
    ...
    ##Save time stats
    dat_telapsed = pd.concat(
        [telapsed_summary_1, telapsed_summary_2, telapsed_summary_3], axis=1
    )
...
for path_ID in dirs:
    ...
    Env_under.path_prePars, Env_over.path_prePars = (
        config_re["path_prePars"],
        config_re["path_prePars"],
    )

    ##Input ################################
    Env_under.n_features, Env_over.n_features = (
        config_re["n_features"][0],
        config_re["n_features"][0],
    )
    Env_under.func_optim, Env_over.func_optim = func_optim_under, func_optim_over

    Env_under.batch_size, Env_over.batch_size = (
        config_re["batch_size"][0],
        config_re["batch_size"][1],
    )
    Env_under.iter_critic, Env_over.iter_critic = (
        config_re["iter_critic"][0],
        config_re["iter_critic"][1],
    )
    ...
    Env_under.transformer = Env_under.get_scaler(
        (sum(1 for line in open(Env_under.path_dat)) - 1), "/real_transformer.pkl"
    )
    ...
    Env_under.n_training_samples = (
        sum(1 for line in open(Env_under.path_dat)) - 1
    )  # number of records in file (excludes header)

    if config["train_main"]:

        Env_under.generator = GanComponent(
            Env_under.n_features,
            Env_under.init_weights,
            config_re["gen_structure"][0][0],
            dropout_prob,
            "generator",
        )
        Env_under.critic = GanComponent(
            Env_under.n_features,
            Env_under.init_weights,
    ...
    n_training_samples = (
        sum(1 for line in open(Env_over.path_dat)) - 1
    )  # number of records in file (excludes header)
    ...
    Env_over.n_training_samples = (
        sum(1 for line in open(Env_under.path_dat)) - 1
    )  # number of records in file (excludes header)

    if config["train_main"]:

        Env_over.generator = GanComponent(
            Env_over.n_features,
    ...
    if config["validate"]:
        ...
        if experiment_data == "microarray":
            prepare_dat(
                PATH + path_ID + "/Data", "dat_real_combo.csv", 1
            )  # remove ID, keep label
        ...
        path_transformer = (
            PATH + path_ID + "/Results/Main_Results/underrep" + "/real_transformer.pkl"
        )
